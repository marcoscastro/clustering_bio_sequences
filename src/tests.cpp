...
void Tests::runHybridSpliceDataTest900Sequences()
{
	...
	for(int test = 0; test < total_tests; test++)
	{
		...
		if(cluster1.size() >= 200 && cluster2.size() >= 200 && cluster3.size() >= 200)
		{
		...
		std::cout << "Class EI : " << cluster1EI << " - " << ((double)cluster1EI / cluster1.size()) * 100.0 << " % \n";
		...
		std::cout << "Class EI : " << cluster2EI << " - " << ((double)cluster2EI / cluster2.size()) * 100.0 << " % \n";
		...
		std::cout << "Class EI : " << cluster3EI << " - " << ((double)cluster3EI / cluster3.size()) * 100.0 << " % \n";
...
void Tests::runNoHybridSpliceDataTest900Sequences()
{
	std::string dataset_name("splice.data.900.sequences.fasta");
	FastaFile ff(dataset_name);
	std::vector<std::pair<std::string, std::string> > dataset;
...
void Tests::runHybridTestsPromotersData()
{
	...
	for(int test = 0; test < total_tests; test++)
	{
		...
		if(cluster1.size() >= 40 && cluster2.size() >= 40)
		{
			if(percent_cluster1_class1 >= 79.9 || percent_cluster1_class2 >= 79.9)
			{
				if(percent_cluster2_class1 >= 79.9 || percent_cluster2_class2 >= 79.9)
				{
					accept++;
				}
			}
		...
		std::cout << "\nCluster 1 - Size : " << cluster1.size() << "\n";
		std::cout << "Class + : " << cluster1_class1 << " - " << ((double)cluster1_class1 / cluster1.size()) * 100.0 << " % \n";
		std::cout << "Class - : " << cluster1_class2 << " - " << ((double)cluster1_class2 / cluster1.size()) * 100.0 << " % \n";

		std::cout << "\nCluster 2 - Size : " << cluster2.size() << "\n";
		std::cout << "Class + : " << cluster2_class1 << " - " << ((double)cluster2_class1 / cluster2.size()) * 100.0 << " % \n";
...
void Tests::runNoHybridTestsPromotersData()
{
	std::string dataset_name("promoters.data.fasta");
	FastaFile ff(dataset_name);
	std::vector<std::pair<std::string, std::string> > dataset;
